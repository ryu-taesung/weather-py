...
class WeatherRadarViewer(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.zip_code_entry.bind(
            '<Return>', lambda e: self.update_gif_periodically(force=True))
        self.zip_code_entry.bind(
            '<KP_Enter>', lambda e: self.update_gif_periodically(force=True))
        self.zip_code_entry.grid(row=0, column=1, padx=5, pady=5)

        fetch_button = ttk.Button(
            frame,
            text="Fetch Radar",
            command=lambda: self.update_gif_periodically(force=True))
        ...
        region_dropdown = tk.OptionMenu(
            frame, self.selected_region, *self.radar_regions)
        ...
        self.image_label.grid(row=1, column=0, columnspan=5,
                              padx=5, pady=5, sticky=(tk.N, tk.E, tk.S, tk.W))
        ...
        self.image_label.bind(
            "<Configure>", lambda event: self.handle_resize())
        self.image_label.bind('<Double-Button-1>',
                              self.restore_image_dimensions)
    ...
    def updated_region(self, *args, **kwargs):
        if self.selected_region.get() != '':
            self.radar_url = self.radar_urls[self.radar_regions.index(
                self.selected_region.get())]
        else:
            self.radar_url = None
    ...
    def fetch_and_display_gif(self, *args, **kwargs):
        ...
        # Step 3: Construct the radar URL with a query string to prevent
        # caching
    ...
    def update_gif_periodically(self, *args, **kwargs):
        ...
        # except:
        #    pass
        self.update_gif_timer = self.after(
            self.refresh_delay, self.update_gif_periodically)
